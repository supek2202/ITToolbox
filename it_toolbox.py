...
class DeviceInspectorApp:
    def __init__(self, root):
        ...
        # macOS .app 包的特殊处理（Resources 目录）
        resources_dir = None
        if getattr(sys, 'frozen', False):
            # 打包后 base_dir 是 MacOS 目录，Resources 在上一层
    ...
    def show_add_device_dialog(self):
        ...
        dialog.geometry(f"+{(dialog.winfo_screenwidth() - 300)//2}+{(dialog.winfo_screenheight() - 340)//2}")

        # 表单容器（place布局，固定尺寸）
        form_frame = tk.Frame(dialog)
        form_frame.place(x=0, y=0, width=300, height=430)

        def row(y, label_text, widget, w_type="entry"):
            tk.Label(form_frame, text=label_text, anchor=tk.W).place(x=10, y=y, width=90, height=24)
            if w_type == "combo":
                widget.place(x=105, y=y, width=200, height=24)
            elif w_type == "proto":
                widget.place(x=105, y=y, width=200, height=24)
            else:
                widget.place(x=105, y=y, width=285, height=24)

        # 设备类型
        device_type = ttk.Combobox(form_frame, values=self.commands_data.get("device_types", []), state="readonly", width=20)
        device_type.place(x=105, y=10, width=185, height=24)
        tk.Label(form_frame, text="设备类型:", anchor=tk.W).place(x=10, y=10, width=90, height=24)
        device_type.current(0)

        # 厂商
        tk.Label(form_frame, text="厂商:", anchor=tk.W).place(x=10, y=40, width=90, height=24)
        vendor = ttk.Combobox(form_frame, values=self.commands_data.get("vendors", []), state="readonly", width=20)
        vendor.place(x=105, y=40, width=185, height=24)
        vendor.current(0)

        # 型号
        tk.Label(form_frame, text="型号:", anchor=tk.W).place(x=10, y=70, width=90, height=24)
        model = tk.Entry(form_frame, width=20)
        model.place(x=105, y=70, width=185, height=24)

        # IP
        tk.Label(form_frame, text="IP地址:", anchor=tk.W).place(x=10, y=100, width=90, height=24)
        ip = tk.Entry(form_frame, width=20)
        ip.place(x=105, y=100, width=185, height=24)

        # 协议/端口
        tk.Label(form_frame, text="协议/端口:", anchor=tk.W).place(x=10, y=130, width=90, height=24)
        protocol_port_frame = tk.Frame(form_frame)
        protocol_port_frame.place(x=105, y=130, width=185, height=24)
        protocol = ttk.Combobox(protocol_port_frame, values=["SSH", "Telnet"], state="readonly", width=9)
        protocol.pack(side=tk.LEFT)
        tk.Label(protocol_port_frame, text="/").pack(side=tk.LEFT, padx=2)
        port = tk.Entry(protocol_port_frame, width=7)
        port.insert(0, "22")
        ...
        # 用户名
        tk.Label(form_frame, text="用户名:", anchor=tk.W).place(x=10, y=160, width=90, height=24)
        username = tk.Entry(form_frame, width=20)
        username.place(x=105, y=160, width=185, height=24)

        # 密码
        tk.Label(form_frame, text="密码:", anchor=tk.W).place(x=10, y=190, width=90, height=24)
        password = tk.Entry(form_frame, show="●", width=20)
        password.place(x=105, y=190, width=185, height=24)

        # 特权密码
        tk.Label(form_frame, text="特权密码:", anchor=tk.W).place(x=10, y=220, width=90, height=24)
        enable = tk.Entry(form_frame, show="●", width=20)
        enable.place(x=105, y=220, width=185, height=24)
        ...
        # 按钮（place布局，放在对话框底部）
        btn_frame = tk.Frame(dialog, bg="#e0e0e0")
        btn_frame.place(x=0, y=270, width=300, height=40)
        tk.Button(btn_frame, text="保存", relief=tk.FLAT, bg="#d0d0d0", command=save).place(x=55, y=8, width=90, height=26)
        tk.Button(btn_frame, text="取消", relief=tk.FLAT, bg="#d0d0d0", command=dialog.destroy).place(x=155, y=8, width=90, height=26)
